...
```
4. 每次更新之前，首先提交代码到远程仓库，然后在本地的 data\_path\_pre 路径下的项目目录中执行git pull更新远程代码到本地
5. 之后分别执行 
```
	bash 0_get_list.sh: 获取存在差异的文件列表
	bash 1_get_diff.sh：查看待更新文件的差异内容
	bash 2_update.sh:执行更新
```
6. 更新完成后，在update/log 目录下会生成更新日志，每个更新日志以update.$timestamp记录
7. 如果需要回滚代码，则找到上一次更新的时间戳，在log目录下可以看到，执行:bash rollback.sh $timestamp 即可
